    ...
    while (*forceExit != true)
    {
        ...
        if (b.has_value())
        {
            ...
            if (MODE == MODE_COMPRESSION)
            {
                ...
                size_t const compressedSize = ZSTD_compressCCtx(cctx, std::get<0>(*compressed).data(), std::get<0>(*compressed).size(), b.value()->first.data(), b.value()->first.size(), 20);
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Usage: " << argv[0] << " <compress | decompress> <input-path> <output-path>" << std::endl;
        exit(-1);
    }
